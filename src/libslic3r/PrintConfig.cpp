...
void PrintConfigDef::init_fff_params()
{
    ...
    def = this->add("internal_walls_flow_ratio", coFloat);
    def->label = L("Walls flow ratio");
    def->category = L("Advanced");
    def->tooltip = L("This factor affects the amount of material for walls. "
                     "\n\nThe actual walls flow used is calculated by multiplying this value with the filament flow ratio, and if set, the object's flow ratio.");
    def->min = 0;
    def->max = 2;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(1));
    ...
    */
    ...
    def->label = L("Ironing uretract extra");
    def->category = L("Quality");
    ...
    def->category = L("Strength");
    def->tooltip = L("This setting adds an extra walls to every other layer. This way the infill gets wedged vertically between the walls, resulting in stronger prints.\n\n"
