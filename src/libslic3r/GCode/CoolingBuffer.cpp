...
struct CoolingLine
{
    ...
    CoolingLine(unsigned int type, size_t  line_start, size_t  line_end) :
        ...
        length(0.f), feedrate(0.f), time(0.f), time_max(0.f), slowdown(false), overhang_degree(-1) {}
...
std::vector<PerExtruderAdjustments> CoolingBuffer::parse_layer_gcode(const std::string &gcode, std::vector<float> &current_pos) const
{
    ...
    for (; *line_start != 0; line_start = line_end) 
    {
        ...
        if (line.type) {
            ...
            size_t at_pos = sline.find('@');
            if (at_pos != std::string::npos)
                line.overhang_degree = std::stoi(sline.substr(at_pos + 1));
            else
                line.overhang_degree = -1;
    ...
    for (const CoolingLine *line : lines) {
        ...
        if (need_set_fan) {
            if (fan_speed_change_requests[CoolingLine::TYPE_OVERHANG_FAN_START]){
                ...
                if (line->overhang_degree >= overhang_fan_threshold)
