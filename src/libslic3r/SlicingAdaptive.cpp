...
{
	float  height = (float)m_slicing_params.layer_height;
...
// @platch's formula for quality:
	    ...
	    double delta_max = (SURFACE_CONST + 0.5) * m_slicing_params.layer_height;
#else
// Vojtech's formula for triangle area error metric.
	    ...
	    double delta_max = m_slicing_params.layer_height;
#endif
...
float SlicingAdaptive::horizontal_facet_distance(float z)
{
	for (size_t i = 0; i < m_faces.size(); ++ i) {
		...
		if (zspan.first > z + m_slicing_params.layer_height)
			break;
	...
	// objects maximum?
	return (z + (float)m_slicing_params.layer_height > (float)m_slicing_params.object_print_z_height()) ? 
		std::max((float)m_slicing_params.object_print_z_height() - z, 0.f) : (float)m_slicing_params.layer_height;
}
