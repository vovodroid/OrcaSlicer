	    ...
	    for (const Surface &surface : layerm.fill_surfaces.surfaces)
	        if (surface.surface_type == stInternalVoid)
                ...
                if (params.extrusion_role == erInternalInfill) {
                    ...
                    params.rotate_angle = (params.pattern == ipRectilinear || params.pattern == ipMonotonic || params.pattern == ipLine || params.pattern == ipMonoCentric);
                } else {
...
//	this->export_region_fill_surfaces_to_svg_debug("10_fill-initial");
    ...
    for (SurfaceFill &surface_fill : surface_fills) {
        ...
        if (surface_fill.params.pattern == ipMonoCentric) {
            rotate_step_bit = 2;
            if (this->id() % 2 == 1)
                surface_fill.params.pattern = ipConcentric;
            else
                surface_fill.params.pattern = ipMonotonicLine;
        }
