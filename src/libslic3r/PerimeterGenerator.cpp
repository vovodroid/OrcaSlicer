...
static void reorient_perimeters(ExtrusionEntityCollection &entities, bool steep_overhang_contour, bool steep_overhang_hole,
                        bool reverse_external, bool reverse_internal)
{
    if (steep_overhang_contour || steep_overhang_hole || reverse_external || reverse_internal) {
        int i = 0;        
            ...
            if (entity->is_loop()) {
                ...
                for(auto path : eloop->paths){
                    if(path.role() == erExternalPerimeter){
                        if (reverse_external || ((eloop->loop_role() & elrHole) == elrHole) ? steep_overhang_hole : steep_overhang_contour) {
                            eloop->reverse();
                            i++;
                        }
                        break;
    ...
    for (const Surface& surface : all_surfaces) {
       ...
       // printf("New Layer: Layer ID %d\n",layer_id); //debug - new layer
        ...
        if (ExtrusionEntityCollection extrusion_coll = traverse_extrusions(*this, ordered_extrusions, steep_overhang_contour, steep_overhang_hole); !extrusion_coll.empty()) {
            ...
            if (config->overhang_reverse || reverse_external || this->config->reverse_internal){
                if (!config->overhang_reverse) {
                ...
                reorient_perimeters(extrusion_coll, steep_overhang_contour, steep_overhang_hole,
                                    reverse_external, this->config->reverse_internal);
            }
