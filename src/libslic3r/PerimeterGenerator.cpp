    ...
    bool detect_threshold_overhang, bool &steep_overhang_contour, bool &steep_overhang_hole)
{
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            if (detect_threshold_overhang && perimeter_generator.config->fuzzy_skin != FuzzySkinType::None) {
                if (loop.is_contour) {
                    steep_overhang_contour = true;
...
void PerimeterGenerator::process_classic()
{
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        if (loop_number >= 0) {
            ...
            bool overhang_reverse = (this->layer_id % 2 == 1) && this->config->detect_overhang_wall && this->config->overhang_reverse;
