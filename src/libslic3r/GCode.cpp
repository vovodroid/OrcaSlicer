...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if (m_config.small_area_infill_flow_compensation.value && !m_config.small_area_infill_flow_compensation_model.empty())
        m_small_area_infill_flow_compensator = make_unique<SmallAreaInfillFlowCompensator>(print.config().small_area_infill_flow_compensation_model);
    ...
    // ORCA: resonance‑avoidance on short external perimeters
{
    ...
    auto append_role_based_fan_marker = [this, &gcode](const ExtrusionRole role, int overhang_degree,  const std::string_view& marker_prefix, const bool fan_on) {
        ...
        if (fan_on) {
            ...
            if (!m_is_role_based_fan_on[role] || m_role_based_fan_marker_layer[role] != m_layer_index || !m_is_role_based_fan_on[role]) {
                gcode += ";";
                gcode += marker_prefix;
